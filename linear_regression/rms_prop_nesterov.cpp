...
class LinearRegression{
    ...
    double slope_gradient(int l,int r){
        double a=0;
        for(int i=l;i<r;i++){
            a+=2*(sl*ind_var[i]+inter+this->mu*this->sl_momentum-dep_var[i])*ind_var[i]/(r-l);
        }
        return a;
    }
    double inter_gradient(int l,int r){
        double a=0;
        for(int i=l;i<r;i++){
            a+=2*(sl*ind_var[i]+inter+this->mu*this->inter_momentum-dep_var[i])/(r-l);
        }
        return a;
    }
    void gradient_descent(){
        for(int j=0;j<this->epochs;j++){
            for(int i=0;i<this->size;i+=this->batch_size){
                ...
                this->sl_m=this->gamma*this->sl_m+(1-this->gamma)*(s_grad*s_grad);
                this->inter_m=this->gamma*this->inter_m+(1-this->gamma)*(i_grad*i_grad);
                this->sl_momentum=this->mu*sl_momentum-learn*s_grad/sqrt(this->sl_m);
                this->inter_momentum=this->mu*inter_momentum-learn*i_grad/sqrt(this->inter_m);
                sl+=sl_momentum;
                inter+=inter_momentum;
            }
        }
    }
    ...
    LinearRegression(double lr,int iter,int batch,double mu,double gamma){
        this->learn=lr;
        this->epochs=iter;
...
int main(){
    ...
    LinearRegression lr(0.001,100,10,0.9,0.9);
    lr.fit(x,y);
    cout<<lr.loss()<<endl;
    cout<<lr.slope()<<endl;
