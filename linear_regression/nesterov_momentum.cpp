...
class LinearRegression{
    ...
    void gradient_descent(){
        for(int j=0;j<this->epochs;j++){
            for(int i=0;i<this->size;i+=this->batch_size){
                ...
                this->sl_momentum=this->mu*sl_momentum-learn*s_grad;
                this->inter_momentum=this->mu*inter_momentum-learn*i_grad;
                sl+=sl_momentum;
    ...
    LinearRegression(double lr,int iter,int batch,double mu){
        this->learn=lr;
        this->epochs=iter;
...
int main(){
    ...
    LinearRegression lr(0.00001,100,10,0.9);
    lr.fit(x,y);
    cout<<lr.loss()<<endl;
    cout<<lr.slope()<<endl;
